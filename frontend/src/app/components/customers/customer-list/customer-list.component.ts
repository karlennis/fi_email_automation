              ...
              <label class="match-card" *ngFor="let p of resendProjects; trackBy: trackMatch" [class.match-card-included]="p.include">
                ...
                <div class="match-card-body">
                  ...
                  <!-- The document is what distinguishes two entries for the same
                       project. Without it they render identically and there is no way
                       to tell which one you are unticking. -->
...
export class CustomerListComponent implements OnInit {
  ...
  /**
   * Track by the entry's own id, not by index: two entries for the same project are
   * otherwise interchangeable to Angular, which can reuse the wrong checkbox DOM.
   */
  trackMatch(_index: number, match: any): string {
    return match?._id || `${match?.projectId}:${match?.metadata?.documentName}`;
  }
